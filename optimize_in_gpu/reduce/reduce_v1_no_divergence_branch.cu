 ...
 * 
 * 问题：
 * 1. 仍然存在 bank conflict：访问模式 shared_data[index] 和 shared_data[index + stride] 可能导致 bank 冲突
 * 2. 线程利用率仍然不高：每次迭代只有部分线程参与计算
 */
__global__ void reduce1(float *device_input, float *device_output){
...
int main(){
    ...
    // 启动 GPU 内核
    reduce1<<<grid_dim, block_dim>>>(device_input_data, device_output_data);
