 ...
 * reduce0: 基础版本的归约实现
 * 问题：
 * 1. 分支发散：if (thread_idx % (2*stride) == 0) 导致同一 warp 内的线程执行不同路径
 * 2. Bank conflict：访问模式可能导致共享内存 bank 冲突
 * 3. 线程利用率低：每次迭代只有部分线程参与计算
 */
__global__ void reduce0(float *device_input, float *device_output){
    ...
    // 在共享内存中进行归约操作
    // 使用树形归约：每次迭代将步长 stride 翻倍，合并相邻的元素对
...
int main(){
    ...
    // 启动 GPU 内核
    reduce0<<<grid_dim, block_dim>>>(device_input_data, device_output_data);
